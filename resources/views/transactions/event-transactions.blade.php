        ...
        <div class="bg-white rounded-lg shadow p-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-yellow-600">{{ $transactions->where('status', 'pending')->count() }}</p>
                </div>
        ...
        <div class="bg-white rounded-lg shadow p-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-green-600">{{ $transactions->where('status', 'paid')->count() }}</p>
                </div>
        ...
        <div class="bg-white rounded-lg shadow p-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xl font-bold text-blue-600">Rp {{ number_format($transactions->where('status', 'paid')->sum('total_amount'), 0, ',', '.') }}</p>
                </div>
                <div class="p-3 bg-blue-100 rounded-lg">
                    <svg class="w-6 h-6 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm">
                                <div class="flex gap-2">
                                    <a href="{{ route('transactions.show', $transaction) }}" class="text-blue-600 hover:text-blue-800 font-medium">
                                        Detail
                                    </a>
                                    @if($transaction->status === 'pending' && $transaction->payment_proof)
                                        <form method="POST" action="{{ route('transactions.verify', $transaction) }}" class="inline">
                                            @csrf
                                            @method('PUT')
                                            <button type="submit" class="text-green-600 hover:text-green-800 font-medium" onclick="return confirm('Verifikasi pembayaran ini?')">
                                                Verifikasi
                                            </button>
                                        </form>
                                    @endif
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-6 py-12 text-center">
                                ...
                                <p class="mt-2 text-gray-500">Belum ada transaksi</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
