    ...
    <div class="grid grid-cols-1 gap-4">
        <div class="p-4 border rounded">
            <div class="flex justify-between items-center mb-4">
                <h2 class="font-semibold">Your Events</h2>
                <a href="/events/create" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded">+ Tambah Event</a>
            </div>
            ...
            <table class="w-full border text-sm">
                ...
                @forelse ($events as $event)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="p-2 border flex flex-wrap gap-2 items-center">
                            {{-- Edit (hanya draft) --}}
                            @if ($event->status === 'draft')
                            ...
                            @if ($event->status === 'draft')
                                <form action="/events/{{ $event->id }}/publish" method="POST">
                                    @csrf
                                    <button class="bg-green-600 hover:bg-green-700 text-white px-3 py-1 rounded text-xs">Publish</button>
                                </form>
                            @endif
                            ...
                            @if ($event->status === 'published')
                                <form action="/events/{{ $event->id }}/cancel" method="POST">
                                    @csrf
                                    <button class="bg-orange-500 hover:bg-orange-600 text-white px-3 py-1 rounded text-xs">Cancel</button>
                                </form>
                            @endif

                            {{-- Delete --}}
                            <form action="/events/{{ $event->id }}" method="POST" onsubmit="return confirm('Yakin hapus event ini?')">
                                @csrf
                                @method('DELETE')
